#!/usr/bin/env python3
"""Generate a package-faithful early residual slice fixture for R1i."""
...
SLICE_NAMES = ["conv1", "layer1.0.conv1", "layer1.0.conv2", "layer1.0.add"]
...
def conv_reference(
    ...
    x = x.view(1, 32, 32, input_c).permute(0, 3, 1, 2).contiguous()
    weight = torch.tensor([to_s8(v) for v in weight_values], dtype=torch.float32)
    weight = weight.view(input_c, 3, 3, output_c).permute(3, 0, 1, 2).contiguous()
    bias = torch.tensor(bias_values, dtype=torch.float32)
    acc = torch.round(F.conv2d(x, weight, bias=bias, stride=1, padding=1)).to(torch.int64)
    if relu:
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", default="tb/generated/resnet20_r1i_package_slice")
    parser.add_argument("--summary", default="tb/generated/resnet20_r1i_package_slice_compare_summary.json")
    parser.add_argument("--sv-include", default="tb/generated/resnet20_r1i_package_slice.vh")
    parser.add_argument("--task-include", default="tb/generated/resnet20_r1i_package_tasks.vh")
    parser.add_argument("--expected-include", default="tb/generated/resnet20_r1i_package_expected.vh")
    ...
    tasks = tasks_doc["tasks"][:4]
    if [task["name"] for task in tasks] != SLICE_NAMES:
        raise SystemExit("package task prefix does not match the R1i contiguous slice")
    tensors = {item["name"]: item for item in memory_doc["tensors"]}

    input_values = seeded_input(int(tensors["input.image"]["byte_size"]))
    write_memh(output / "input_image.memh", input_values, 8)
    current = input_values
    stages: list[dict[str, Any]] = []

    for stage_idx, task in enumerate(tasks[:3]):
        name = task["name"]
        ...
        current = conv_reference(
            current, input_c, output_c, weight_values, bias_values,
        ...
        bias_bytes = [((value & 0xFFFFFFFF) >> (8 * lane)) & 0xFF
                      for value in bias_values for lane in range(4)]
        write_memh(output / f"{stem}_bias_bytes.memh", bias_bytes, 8)
    ...
    add_task = tasks[3]
    add_plan = find_item(add_doc, add_task["name"])
    shortcut = [int(line.strip(), 16) for line in (output / "conv1_expected.memh").read_text(encoding="ascii").splitlines()]
    main = torch.tensor([to_s8(v) for v in current], dtype=torch.int64)
    short = torch.tensor([to_s8(v) for v in shortcut], dtype=torch.int64)
    main_aligned = requant(main, add_plan["main_to_target"], clamp=True)
    shortcut_aligned = requant(short, add_plan["shortcut_to_target"], clamp=True)
    add_relu = torch.clamp(main_aligned + shortcut_aligned, min=0)
    post_plan = search_multiplier_shift(float(add_plan["target_add_scale"]) / float(add_plan["post_relu_scale"]))
    add_output = [int(value) & 0xFF for value in requant(add_relu, post_plan, clamp=True).tolist()]
    write_memh(output / "layer1_0_add_expected.memh", add_output, 8)
    stages.append({
        "stage_index": 3,
        "task_id": add_task["task_id"],
        "name": add_task["name"],
        "task_type": 4,
        "input_addr": add_task["memory"]["inputs"]["layer1.0.conv2.pre_add_main"],
        "src1_addr": add_task["memory"]["inputs"]["conv1.relu"],
        "output_addr": add_task["memory"]["output"],
        "input_bytes": tensors["layer1.0.conv2.pre_add_main"]["byte_size"],
        "src1_bytes": tensors["conv1.relu"]["byte_size"],
        "output_bytes": tensors["layer1.0.add.relu"]["byte_size"],
        "input_c": 16,
        "output_c": 16,
        "add_cfg": 0xC,
        ...
        "expected_file": str(output / "layer1_0_add_expected.memh"),
    ...
    summary = {
        ...
        "input_addr": tensors["input.image"]["base_addr"],
    ...
    lines = ["// Generated by extract_resnet20_r1i_package_slice.py"]
    lines += [
        f"localparam [31:0] R1I_INPUT_ADDR = 32'd{summary['input_addr']};",
        f"localparam [31:0] R1I_WEIGHT_ADDR = 32'd{WEIGHT_LOAD_ADDR};",
        f"localparam [31:0] R1I_BIAS_ADDR = 32'd{BIAS_LOAD_ADDR};",
    ]
    for item in stages:
        prefix = f"R1I_S{item['stage_index']}"
        ...
        if item["task_type"] == 0:
            ...
            for key in ["src1_addr", "src1_bytes", "add_cfg", "src0_multiplier", "src0_shift", "src1_multiplier", "src1_shift", "out_multiplier", "out_shift"]:
                lines.append(f"localparam [31:0] {prefix}_{key.upper()} = 32'd{item[key]};")
    ...
    tensor_names = ["input.image", "conv1.relu", "layer1.0.conv1.relu", "layer1.0.conv2.pre_add_main", "layer1.0.add.relu"]
    ...
    for item in stages:
        ...
        op = "CONV3x3" if item["task_type"] == 0 else "RESIDUAL_ADD"
        task_lines += [
            ...
            f"  r1f_input_h[{idx}] = 32'd32; r1f_input_w[{idx}] = 32'd32;",
    ...
    expected_lines = [
        ...
        f"  $readmemh(\"{output / 'input_image.memh'}\", r1i_load_byte);",
    ...
    print(f"scope={','.join(SLICE_NAMES)}")
    return 0
