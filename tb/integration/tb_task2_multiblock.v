// tb_task2_multiblock: verify multi-block Conv with block_scheduler
// Input: 50×5 (250 bytes, all 1s), Kernel: 5×5 (25 bytes, all 2s)
// Output: 46×1 (184 bytes INT32), with BUF_ENTRIES=128 should need multiple blocks
`timescale 1ns / 1ps

module tb_task2_multiblock;
    reg clk, rst_n;
    ...
    // Use medium buffer to force multi-block (128 entries = 512 bytes per bank, 2 blocks)
    ...
    initial begin
        $dumpfile("sim/tb_task2_multiblock.vcd");
        $dumpvars(0, tb_task2_multiblock);
        ...
        blk_count = 0; errors = 0;

        #20 rst_n=1; #20;

        // Pre-load activations: 30×5 = 150 bytes (38 words, all 0x01010101)
        $display("=== Pre-load activations (30x5, all 1s) ===");
        for (i = 0; i < 38; i = i + 1)
            preload_word(32'h100 + i*4, 32'h01010101);

        // Pre-load weights: 5×5 = 25 bytes (7 words, all 0x02020202)
        ...
        // Configure NPU
        $display("=== Configure Conv: 30x5 input, 5x5 kernel → 26x1 output (2 blocks: 20+6) ===");
        ...
        axi_write(32'h1000_0020, 32'h0000_0068);  // output_bytes=104 (26×INT32)
        // H=30 in [15:0], W=5 in [31:16]
        ...
        if (rd_val[2]) begin
            $display("PASS: done=1");
            // Check first and last output — each should be 50
            // Read output[0] at 0x300 (first output row)
            axi4_read(32'h300, rd_data);
            if ($signed(rd_data) !== 50) begin errors = errors + 1; $display("  FAIL: output[0]=%0d", $signed(rd_data)); end
            else $display("  PASS: output[0]=50");

            // Read output[25] at 0x300 + 25*4 = 0x364 (last output row)
            axi4_read(32'h300 + 25*4, rd_data);
            if ($signed(rd_data) !== 50) begin errors = errors + 1; $display("  FAIL: output[25]=%0d", $signed(rd_data)); end
            else $display("  PASS: output[25]=50");
            ...
            $display("FAIL: Still busy after 100000 cycles, FSM=%0d sc=%0d ad=%b",
                u_npu.fsm_state, u_npu.u_block_sched.curr_out_row, u_npu.blk_all_done);
            errors = errors + 1;
        end

        if (errors == 0) $display("=== FINAL: ALL PASS ===");
        else $display("=== FINAL: %0d ERRORS - FAIL ===", errors);
        #20 $finish;
    end
endmodule
