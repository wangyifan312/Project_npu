// array_top：由 4×4 MAC tile 组成的脉动阵列，iverilog 兼容的扁平端口
...
module array_top #(
    ...
    // 辅助函数：从扁平索引获取 tile 行列
    // 扁平索引 = tr × TILE_COLS + tc

    // 内部连线：tile 间水平激活（扁平）
    // act_h[(tr*TILE_COLS+tc)*4+k] = tile(tr,tc) PE 行 k 的激活，进入该 tile
    // tc 中 +1 偏移表示该 tile 之后 = 下一个 tile 的输入
